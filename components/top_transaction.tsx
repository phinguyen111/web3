...
const fetchTransactions = async () => {
    ...
    const response = await fetch(`https://api.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&sort=desc&apikey=${process.env.NEXT_PUBLIC_API_KEY}`);
    ...
    // Convert data into an array of Transaction objects
    ...
    // Sort and get the top 10 largest transactions
    const sortedTransactions = transactions.sort((a, b) => b.totalEth - a.totalEth).slice(0, 10);

    // Assign rank to each transaction after sorting
