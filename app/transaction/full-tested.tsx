...
export default function TransactionExplorer() {
  ...
  const fetchStatistics = async () => {
    try {
      ...
      if (txData.result) {
        ...
        // Fetch transaction count for the block from one day ago
        ...
        if (txCountData.result) {
          ...
          const transactionsResponse = await fetch(`https://api.etherscan.io/api?module=proxy&action=eth_getBlockByNumber&tag=${oneDayAgoBlock.toString(16)}&boolean=true&apikey=${ETHERSCAN_API_KEY}`);
          const transactionsData = await transactionsResponse.json();

          if (transactionsData.result && transactionsData.result.transactions) {
            // Sum the amounts of the transactions
            totalTransactionAmount = transactionsData.result.transactions.reduce((acc: number, tx: any) => {
              return acc + parseFloat(ethers.utils.formatEther(tx.value));
            }, 0);
          }
        }
      }
      ...
      setStats({
        ...
        totalTransactionAmount: totalTransactionAmount, // Update total transaction amount
      });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchStatistics(); // Update every 5 minutes
